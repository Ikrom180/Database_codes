CREATE OR REPLACE VIEW ANTIFRAUD.v_gl_cards_for_antifraud as 
SELECT 'pc' as source,
c.id ||'.'||c.client_code as source_id,
c.client_code as cid,
c.contract_id as pc_id,
c.gl_account_number as account_id,
c.gl_card_acct as account,
c.id as card_id,
'Humo' as sub_type_name,
...
c.gl_state_code as status_code,
DECODE(c.gl_state_code, '0','Активный','1','Украденный','2','Потерянный','3','Нет денег','4','Pick-up','5','Нечестный','6','Не активный','A','По желанию банка','B','По желанию картодержателя','H','Closed by TSM wallet','S','Suspended by TSM',c.gl_state_code) as status_name,
