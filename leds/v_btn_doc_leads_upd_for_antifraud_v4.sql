CREATE OR REPLACE VIEW ANTIFRAUD.v_btn_doc_leads_upd_for_antifraud as
...
l.debit_acc as payer_account, /*Плательщик. Номер счета*/
...
l.credit_acc as receiver_account, /*Получатель. Номер счета*/
...
l.doc_id as ext_id, /*ID платежа во внешней системе*/
...
JOIN ibs.btn_doc_leads l on l.lead_id = c.id
...
WHERE c.id in (select lead_id from ibs.lead_protocol_cur where time >= SYSDATE - 1/144)
;